...
function PersonnelSwap({
    ...
    return (
      <div className="flex items-center gap-1.5 text-right" dir="rtl">
        ...
        <div className="flex flex-col min-w-0">
          <span className="font-bold text-sm truncate">{currentName || "-"}</span>
          {currentName && person?.role && (
            <span className="text-[9px] text-muted-foreground font-medium leading-none mt-0.5 truncate">{person.role}</span>
          )}
        </div>
      </div>
    );
  ...
  const trigger = (
    <button className={cn(
      "flex items-center gap-1.5 hover:text-primary transition-colors text-right group min-w-0",
      ...
      <div className="flex flex-col items-start min-w-0 text-right">
        ...
        {currentName && person?.role && (
          <span className="text-[9px] text-muted-foreground/70 font-medium truncate max-w-[90px] leading-none mt-0.5">
            {person.role}
          </span>
        )}
      </div>
